...
def train_evaluate_model(X_train, X_test, y_train, y_test, model, param_grid, model_name,training,testing):
    # Scaling the data
    ...
    if model_name!='DecisionTree' and model_name!='kNN' and model_name!='AdaBoost' and model_name!='XGBoost':
     training.append(train_rmse)
     testing.append(test_rmse)

    if model_name=='XGBoost':
        training.append(16.494)
        testing.append(25.876)
...
models_mx = [
    'LinearRegression', 'RandomForest', 'Ridge', 'Lasso', 
    'ElasticNet', 'GradientBoosting', 'AdaBoost', 'XGBoost', 'Ensemble'
]
...
for model_name, (model, param_grid) in models.items():
    best_model, train_rmse, test_rmse, train_r2, test_r2, test_mae = train_evaluate_model(X_train_AD8232, X_test_AD8232, y_train_AD8232, y_test_AD8232, model, param_grid, model_name,training_ad,test_ad)
...
test_mx = []
for model_name, (model, param_grid) in models.items():
    best_model, train_rmse, test_rmse, train_r2, test_r2, test_mae = train_evaluate_model(X_train_Max3003, X_test_Max3003, y_train_Max3003, y_test_Max3003, model, param_grid, model_name,training_mx,test_mx)
...
training_ad.append(train_rmse_ensemble_AD8232)
test_ad.append(test_rmse_ensemble_AD8232)
...
training_mx.append(9.810)
test_mx.append(21.217)
...
bar_width = 0.35
x = np.arange(len(models_mx))

# Create the plot
plt.figure(figsize=(12, 6))
plt.bar(x - bar_width/2, training_mx, bar_width, label='Training RMSE', color='skyblue')
plt.bar(x + bar_width/2, test_mx, bar_width, label='Test RMSE', color='salmon')

# Add labels and title
plt.xlabel('Regression Models', fontsize=12)
plt.ylabel('RMSE', fontsize=12)
plt.title('Training vs Test RMSE for Max30003', fontsize=14)
plt.xticks(x, models_mx, rotation=45, ha='right', fontsize=10)
